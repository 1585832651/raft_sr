...
class RaftRestorationModel(BaseModel):
    ...
    def setup_optimizers(self):
        ...
        if optim_type == 'Adam':
            ...
            self.optimizer_g = Lion([{'params': optim_params}],
                                    **train_opt['optim_g'])
            pass
        else:
    ...
    def feed_data(self, data, is_val=False):
        ...
        if 'h_pad' in data:
            self.h_pad = data['h_pad'] 
            self.w_pad = data['w_pad'] 
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        if self.cri_perceptual:
            l_precep_l, l_style_l = self.cri_perceptual(self.output[:,:3], self.gt[:,:3])
            l_percep_r, l_style_r = self.cri_perceptual(self.output[:,3:], self.gt[:,3:])
            if l_precep_l is not None:
                l_percep = l_precep_l+l_percep_r
            else:
                l_percep = None
